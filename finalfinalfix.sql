...
BEGIN
    ...
    -- Update endDate and reassign for sales representatives whose assignment ends today
    UPDATE salesRepAssignments
    SET endDate = NOW() + INTERVAL 1 WEEK
    WHERE endDate = currentDate;

    -- Recompute and update quota for affected sales representatives
    UPDATE customers c
    JOIN (
        SELECT sra.employeeNumber, 
               IF(SUM(po.amountPaid) IS NULL, 0, SUM(po.amountPaid)) AS facilitatedSales
        FROM salesRepAssignments sra
        LEFT JOIN orders o ON sra.employeeNumber = o.salesRepEmployeeNumber
        LEFT JOIN payment_orders po ON o.orderNumber = po.orderNumber
        WHERE sra.endDate = currentDate
        GROUP BY sra.employeeNumber
    ) AS s ON c.salesRepEmployeeNumber = s.employeeNumber
    SET c.creditLimit = c.creditLimit - s.facilitatedSales;

    -- Record the reassignment as system-processed
    UPDATE salesRepAssignments
    SET salesManagerNumber = salesManager
    WHERE endDate = NOW() + INTERVAL 1 WEEK;
END$$
